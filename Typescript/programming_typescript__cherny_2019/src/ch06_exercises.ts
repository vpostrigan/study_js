...
// 1. For each of the following pairs of types,
// decide if the first type is assignable to the second type, and why or why not.
// Think about these in terms of subtyping and variance,
// and refer to the rules at the start of the chapter if you’re unsure
...
let e: (number | string)[]
e = [1] as number[]
/* Yes. Arrays are covariant in their members, so for an array to be assignable to another array,
 its members needs to be <: the other array's members.
 number is a subtype of number | string, so number[] is assignable to (number | string)[]. */

// 1f. (number | string)[] and number[]
let f: number[]
f = [1] as (number | string)[]
/* No. Arrays are covariant in their members, so for an array to be assignable to another array,
 its members needs to be <: the other array's members.
 number | string is a supertype of number, rather than a subtype,
 so (number | string)[] is not assignable to number[]. */

// 1g. {a: true} and {a: boolean}
let g: {a: boolean}
g = {a: true} as {a: true}
/* Yes. Objects are covariant in their members, so for an object to be assignable to another object,
 each of its members needs to be <: the type of the other object's members.
 This object has just one member, a, and its type is the type literal true.
 true is <: boolean, so the whole object {a: true} is assignable to {a: boolean}. */

// 1h. {a: {b: [string]}} and {a: {b: [number | string]}}
let h: {a: {b: [number | string]}}
h = {a: {b: ['c']}} as {a: {b: [string]}}
/* Yes. Combining the rules from (e) and (g), for a nested object to be assignable to another object,
 each of its members needs to be <: the other object's members. We repeat this recursively:
...
/* Yes. For a function to be assignable to another function,
 ...
 number is >: number, and string is <: string, so the function type is assignable. */

// 1j. (a: number) => string and (a: string) => string
let j: (a: string) => string
j = ((a: number) => 'b') as (a: number) => string

/* No. For a function to be assignable to another function,
 each of its parameters should be >: the other function's parameters,
 and its return type should be <: the other function's return type.
 number is unrelated to string, so it's not >: string, implying that the function type isn't assignable. */
...
/* Yes. For a function to be assignable to another function,
 each of its parameters should be >: the other function's parameters,
 and its return type should be <: the other function's return type.
 number | string is a supertype of string, and string is <: string, so the function type is assignable. */
...
/* No. For an enum member to be assignable to another enum member, it has to come from the same enum.
 Though both members are named X and have the same string value X,
 because they're defined on different enums, they are not assignable to one another. */

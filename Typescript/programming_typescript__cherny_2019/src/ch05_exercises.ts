...
// 2. When you mark a class' constructor as `private`, that means you can't instantiate or extend the class.
...
/* Unlike a class with a private constructor, a class with a protected constructor can be extended.
  Neither a class with a private constructor nor a class with a protected constructor can be new-ed. */

// 3. Extend the Factory Pattern implementation we developed (Factory Pattern) to make it safer,
// at the expense of breaking the abstraction a bit.
// Update the implementation so that a consumer knows at compile time that calling Shoe.create('boot') returns a Boot,
// and Shoe.create('balletFlat') returns a BalletFlat (rather than both returning a Shoe). Hint: Think back to [function-overloads].
...
// Extend the Builder pattern example from earlier in this chapter to:

// 4a. Guarantee at compile time that someone can’t call .send() before setting at least URL and method.
